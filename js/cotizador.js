...
function initCotizador() {
  ...
  function calculateSummaryTotals() {
    ...
    coverage = Math.min(Math.max(coverage, 0), 100);
    const hasInsurer = aseguradora && aseguradora !== 'Particular';
    const copagoPercent = hasInsurer ? 100 - coverage : 0;
    ...
    cart.forEach((item) => {
      ...
      if (hasInsurer) {
        if (exceptions.includes(item.codigo)) {
          ...
          copagoReferencial += pvaUnit * item.cantidad * (copagoPercent / 100);
        }
      }
    });
    ...
    const totalPagar = hasInsurer
      ? copagoReferencial + totalSinCobertura
      : subtotalPva;
